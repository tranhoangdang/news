<?php
    ...
    if(isset($_POST['add'])){
        ...
        if($result->num_rows > 0){
            ...
            $sql = "INSERT INTO categories(name,is_active,adminID) VALUES ('$name','$status','$admin')";
            if($conn->query($sql) === TRUE){
                set_flash_session('mess_flash','Chuyên mục ' . $name . ' đã được thêm.');
                redirect('add-category.php');
            } else {
                set_flash_session('error','Không thể thêm chuyên mục này.');
            }
        }
    }
...
<head>
    ...
    <title>Thêm chuyên mục | Quản trị viên</title>
                        ...
                        <form method="POST">
                            <div class="col-lg-6 col-12 m-auto">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="name" name="name" aria-describedby="name" placeholder="Nhập tên chuyên mục" required>
                                </div>
                                <div class="mb-3">
